...
A `tools` list declares the software the derived files are made with -- the
Blender extension the people are generated by -- as single files with
`source`, `sha256` and `bytes`; fetched and verified like an asset, never
compiled, and never part of what the scene draws.
...
def main() -> int:
    ...
    if command == "fetch-list":
        ...
        for tool in manifest.get("tools", []):
            print("\t".join([tool["name"], tool["source"], tool["file"], tool["sha256"],
                             str(tool["bytes"])]))
        return 0
